...
const UniversityApp = {
    ...
    // Load JSON config (absolute path)
    async loadConfig() {
        try {
    ...
    renderFields(fields, container, emptyState) {
        ...
        fields.forEach(field => {
            ...
            fieldCard.href = `/majors?field=${encodeURIComponent(field.code)}`;
            ...
            if (!hasFormations) {
                majorCard.innerHTML = `
                    ...
                    <div class="formations-list">
                        <div class="formation-item no-program">
                            <div class="formation-header">
                                <div class="formation-type">Programme</div>
                                <div class="formation-badge coming-soon">À venir</div>
                            </div>
                        </div>
                    </div>
                `;
                ...
                const formationsList = major.formations.map(formation => {
                    const specialtiesText = formation.specialties && formation.specialties.length > 0
                        ? `${formation.specialties.length} spécialité${formation.specialties.length > 1 ? 's' : ''}`
                        : 'Programme';
                    return `
                        <div class="formation-item">
                            <div class="formation-header">
                                <div class="formation-type">${formation.type}</div>
                                <div class="formation-badge">${specialtiesText}</div>
                            </div>
                            ${formation.programPath
                                ? `<a href="/program?path=${encodeURIComponent(formation.programPath)}" class="program-link">Voir le programme →</a>`
        ...
        try {
            ...
            document.querySelector('.container').innerHTML = `
                <div class="empty-state show">
                    ...
                    <a href="/" style="margin-top: 20px; display: inline-block; padding: 10px 20px; background: #f59e0b; color: white; text-decoration: none; border-radius: 8px; font-weight: 600;">Retour</a>
                </div>
            `;
        }
    },
        ...
        semesters.forEach(semester => {
            ...
            const unitsHtml = semester.units.map(unit => {
                const subjectsHtml = unit.subjects.map(subject => `
                    <div class="subject-item">
                        ...
                        <div class="subject-detail"><span class="subject-label">Credit</span><span class="subject-value">${subject.credit}</span></div>
                        <div class="subject-detail"><span class="subject-label">Coef.</span><span class="subject-value">${subject.coef}</span></div>
                    ...
                    <div class="unit-card">
                        <div class="unit-header">
                            ...
                            <div class="unit-stats">
                                <div class="stat-badge"><span class="stat-value">${unit.credit}</span><span class="stat-label">Credits</span></div>
                                <div class="stat-badge"><span class="stat-value">${unit.Coefficients}</span><span class="stat-label">Coef.</span></div>
                            </div>
                        </div>
            ...
            semesterDiv.innerHTML = `
                <div class="semester-header">
                    <div class="semester-title">Semestre ${semester.semester}</div>
                    <div class="semester-info">${totalCredits} Crédits • ${totalCoef} Coefficients • ${semester.units.length} Unités</div>
                </div>
